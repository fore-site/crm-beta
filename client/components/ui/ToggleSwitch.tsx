...
const ToggleSwitch: React.FC<ToggleSwitchProps> = ({ checked, onChange, labelLeft, labelRight }) => {
  ...
  return (
    <label className="inline-flex items-center cursor-pointer">
      {labelLeft && <span className="mr-3 text-sm font-medium text-gray-900">{labelLeft}</span>}
      ...
      {labelRight && <span className="ml-3 text-sm font-medium text-gray-900">{labelRight}</span>}
    </label>
  );
};
