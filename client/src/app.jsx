...
export default function App() {
  ...
  return (
    <div className="bg-slate-50 min-h-screen font-sans antialiased text-gray-800 p-4 sm:p-6 md:p-8">
      {/* Navigation bar */}
      <nav className="flex items-center justify-between bg-white shadow-lg rounded-xl p-4 sm:p-6 mb-8">
        <h1 className="text-xl sm:text-2xl md:text-3xl font-extrabold text-indigo-700 tracking-tight">Ad CRM</h1>
        <div className="flex space-x-2 sm:space-x-4">
          <button
            onClick={() => setCurrentPage('dashboard')}
            className={`px-3 py-2 sm:px-4 sm:py-2 rounded-lg font-semibold transition-all duration-300 ${currentPage === 'dashboard' ? 'bg-indigo-600 text-white shadow-md' : 'text-gray-600 hover:bg-slate-100 hover:text-indigo-600'}`}
          >
            Dashboard
          </button>
          <button
            onClick={() => setCurrentPage('clients')}
            className={`px-3 py-2 sm:px-4 sm:py-2 rounded-lg font-semibold transition-all duration-300 ${currentPage === 'clients' ? 'bg-indigo-600 text-white shadow-md' : 'text-gray-600 hover:bg-slate-100 hover:text-indigo-600'}`}
          >
            Clients
          </button>
          <button
            onClick={() => setCurrentPage('campaigns')}
            className={`px-3 py-2 sm:px-4 sm:py-2 rounded-lg font-semibold transition-all duration-300 ${currentPage === 'campaigns' ? 'bg-indigo-600 text-white shadow-md' : 'text-gray-600 hover:bg-slate-100 hover:text-indigo-600'}`}
          >
            Campaigns
          </button>
        </div>
...
const Dashboard = ({ clients, campaigns }) => {
  ...
  return (
    <div className="space-y-8 md:space-y-10">
      <h2 className="text-3xl sm:text-4xl font-bold text-gray-700 mb-6">Overview</h2>

      {/* Stats cards */}
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6 sm:gap-8">
        <div className="bg-white p-6 sm:p-8 rounded-xl shadow-lg hover:shadow-2xl transition-all duration-300 transform hover:scale-105">
          <h3 className="text-lg sm:text-xl font-semibold text-gray-600">Total Clients</h3>
          <p className="mt-2 text-4xl sm:text-5xl font-extrabold text-indigo-600">{clients.length}</p>
        </div>
        <div className="bg-white p-6 sm:p-8 rounded-xl shadow-lg hover:shadow-2xl transition-all duration-300 transform hover:scale-105">
          <h3 className="text-lg sm:text-xl font-semibold text-gray-600">Total Campaigns</h3>
          <p className="mt-2 text-4xl sm:text-5xl font-extrabold text-teal-600">{campaigns.length}</p>
        </div>
      </div>

      {/* Recent Campaigns section */}
      <div className="bg-white p-6 sm:p-10 rounded-xl shadow-lg">
        <h3 className="text-xl sm:text-2xl font-semibold text-gray-700 mb-6">Recent Campaigns</h3>
        {recentCampaigns.length > 0 ? (
          <ul className="space-y-4">
            {recentCampaigns.map((campaign, index) => (
              <li key={index} className="border-b border-gray-200 pb-4 last:border-b-0">
                <p className="text-lg font-medium text-gray-700">{campaign.campaignName}</p>
                <p className="text-sm text-gray-500 mt-1">Created: {new Date(campaign.createdAt).toLocaleDateString()}</p>
              </li>
            ))}
          </ul>
        ) : (
          <p className="text-gray-500">No recent campaigns. Create a new one!</p>
        )}
      </div>
    </div>
  );
...
const Clients = ({ clients, setClients }) => {
  ...
  const handleFormSubmit = async (e) => {
    ...
    try {
      if (editingClientId) {
        // Handle update logic
        const res = await fetch(`${API_BASE_URL}/api/clients/${editingClientId}`, {
          method: 'PUT',
        ...
        setClients(prevClients => prevClients.map(c => c._id === updatedClient._id ? updatedClient : c));
    ...
    reader.onload = async (event) => {
      ...
      try {
        if (fileType === 'json') {
          ...
          setMessage('Unsupported file type. Please upload a JSON or CSV file.');
          return;
        }

        for (const client of clientsToUpload) {
            if (!client.name || !client.email || !client.phone) {
                setMessage(`File upload failed: Client data must include name, email, and phone.`);
                return;
            }
            if (!validateEmail(client.email)) {
                setMessage(`File upload failed: Invalid email format for client "${client.name}".`);
                return;
            }
        }
        ...
        setMessage('Failed to upload clients. Please check file format and data.');
      }
    };
  ...
  const confirmDelete = async () => {
    try {
      const res = await fetch(`${API_BASE_URL}/api/clients/${clientToDeleteId}`, {
        method: 'DELETE'
      });
  ...
  return (
    <div className="space-y-8 md:space-y-10">
      <h2 className="text-3xl sm:text-4xl font-bold text-gray-700 mb-6">{editingClientId ? 'Edit Client' : 'Add Client'}</h2>

      {message && (
        <div className="bg-indigo-100 border-l-4 border-indigo-500 text-indigo-700 p-4 rounded-lg" role="alert">
          <p>{message}</p>
        </div>
      )}
      ...
      {showDeleteModal && (
        <div className="fixed inset-0 bg-gray-600 bg-opacity-50 overflow-y-auto h-full w-full flex items-center justify-center z-50">
          <div className="bg-white p-6 sm:p-8 rounded-xl shadow-2xl max-w-sm mx-auto transform scale-95 md:scale-100 transition-all duration-300">
            ...
            <div className="flex justify-end space-x-4">
              <button
                ...
                className="px-4 sm:px-6 py-2 sm:py-3 rounded-lg font-semibold text-gray-700 bg-gray-200 hover:bg-gray-300 transition-colors"
              >
                Cancel
              </button>
              <button
                onClick={confirmDelete}
                className="px-4 sm:px-6 py-2 sm:py-3 rounded-lg font-semibold text-white bg-red-600 hover:bg-red-700 transition-colors"
              >
                Delete
              </button>
            </div>
      ...
      <div className="bg-white p-6 sm:p-10 rounded-xl shadow-lg">
        <h3 className="text-xl sm:text-2xl font-semibold text-gray-700 mb-6">{editingClientId ? 'Edit Client' : 'Add Client Manually'}</h3>
        <form onSubmit={handleFormSubmit} className="space-y-4 sm:space-y-6">
          <input
            ...
            className="w-full p-3 sm:p-4 border border-gray-300 rounded-lg focus:outline-none focus:ring-2 focus:ring-indigo-500 transition-colors"
          />
          <input
            type="email"
            placeholder="Email"
          ...
          <div className="flex flex-col sm:flex-row space-y-4 sm:space-y-0 sm:space-x-4">
            <button
              type="submit"
              className="flex-1 bg-indigo-600 text-white p-3 sm:p-4 rounded-lg font-semibold hover:bg-indigo-700 transition-colors shadow-md hover:shadow-lg"
            >
            ...
            {editingClientId && (
              <button
                type="button"
                onClick={resetForm}
                className="flex-1 bg-gray-400 text-white p-3 sm:p-4 rounded-lg font-semibold hover:bg-gray-500 transition-colors shadow-md hover:shadow-lg"
              >
                Cancel Edit
              </button>
            )}
      ...
      {/* File Upload Section */}
      <div className="bg-white p-6 sm:p-10 rounded-xl shadow-lg">
        <h3 className="text-xl sm:text-2xl font-semibold text-gray-700 mb-6">Upload Clients (JSON or CSV)</h3>
        <input
          type="file"
          accept=".json, .csv"
      ...
      {/* All Clients List */}
      <div className="bg-white p-6 sm:p-10 rounded-xl shadow-lg">
        <h3 className="text-xl sm:text-2xl font-semibold text-gray-700 mb-6">All Clients</h3>
        {clients.length > 0 ? (
          <ul className="space-y-4">
            {clients.map((client) => (
              <li key={client._id} className="flex flex-col md:flex-row md:items-center justify-between p-4 sm:p-6 bg-slate-50 rounded-lg shadow-sm hover:shadow-md transition-shadow duration-300">
                <div className="flex-1">
                  <p className="text-base sm:text-lg font-medium text-gray-700">{client.name}</p>
                  <p className="text-xs sm:text-sm text-gray-500 mt-1">Email: {client.email}</p>
                  <p className="text-xs sm:text-sm text-gray-500">Phone: {client.phone}</p>
                </div>
          ...
          <p className="text-gray-500">No clients found. Add one above!</p>
        )}
      </div>
    </div>
  );
...
const Campaigns = ({ clients, campaigns, setCampaigns }) => {
  ...
  const handleCreateCampaign = async (e) => {
    ...
    try {
      if (editingCampaignId) {
        ...
        const res = await fetch(`${API_BASE_URL}/api/campaigns/${editingCampaignId}`, {
          method: 'PUT',
          headers: { 'Content-Type': 'application/json' },
          body: JSON.stringify({ campaignName, advertTitle, message, imageUrl })
        });
  ...
  const confirmDelete = async () => {
    try {
      const res = await fetch(`${API_BASE_URL}/api/campaigns/${campaignToDeleteId}`, {
        method: 'DELETE'
      });
  ...
  const handleSendAd = async (campaign) => {
    ...
    if (clients.length === 0) {
      setSendConfirmation({
        ...
        message: "Cannot send. There are no clients to send to. Please add clients first."
      });
      return;
    }
    ...
    try {
      ...
      const updatedCampaign = await res.json();
      setCampaigns(prev => prev.map(c => c._id === updatedCampaign.campaign._id ? updatedCampaign.campaign : c));
      ...
      setSendConfirmation({
        campaignName: campaign.campaignName,
        message: "An error occurred while sending the campaign. Please check the console for details."
      });
    } finally {
      setIsSending(false);
    }
  ...
  return (
    <div className="space-y-8 md:space-y-10">
      <h2 className="text-3xl sm:text-4xl font-bold text-gray-700 mb-6">{editingCampaignId ? 'Edit Campaign' : 'Create Campaign'}</h2>
      {campaignMessage && (
      ...
      {sendConfirmation && (
        <div className="fixed inset-0 bg-gray-600 bg-opacity-50 overflow-y-auto h-full w-full flex items-center justify-center z-50">
          <div className="bg-white p-6 sm:p-8 rounded-xl shadow-2xl max-w-sm mx-auto transform scale-95 md:scale-100 transition-all duration-300">
            <h3 className="text-xl font-bold text-gray-800 mb-4">Campaign Status</h3>
            <p className="text-gray-600 mb-6">{sendConfirmation.message}</p>
            <button
              onClick={() => setSendConfirmation(null)}
              className="w-full bg-indigo-600 text-white p-3 sm:p-4 rounded-lg font-semibold hover:bg-indigo-700 transition-colors shadow-md hover:shadow-lg"
            >
              Close
            </button>
          </div>
      ...
      {/* Campaign Form */}
      <div className="bg-white p-6 sm:p-10 rounded-xl shadow-lg">
        <h3 className="text-xl sm:text-2xl font-semibold text-gray-700 mb-6">{editingCampaignId ? 'Edit Campaign' : 'Create Campaign'}</h3>
        <form onSubmit={handleCreateCampaign} className="space-y-4 sm:space-y-6">
          <input
            type="text"
          ...
          {/* Image URL input */}
          <p className="text-gray-500 text-sm">Paste a public image URL here for best results in emails.</p>
          <input
            type="url"
          ...
          <div className="flex flex-col md:flex-row space-y-4 md:space-y-0 md:space-x-4">
            <button
              ...
              className="flex-1 bg-gray-500 text-white p-3 sm:p-4 rounded-lg font-semibold hover:bg-gray-600 transition-colors shadow-md hover:shadow-lg"
            >
              Show Previews
            </button>
            <button
          ...
          {editingCampaignId && (
            <button
              type="button"
              onClick={resetForm}
              className="w-full mt-4 bg-gray-400 text-white p-3 sm:p-4 rounded-lg font-semibold hover:bg-gray-500 transition-colors shadow-md hover:shadow-lg"
            >
              Cancel Edit
            </button>
          )}
      ...
      {(emailPreview || smsPreview || telegramPreview) && (
        <div className="space-y-6">
          <h3 className="text-xl sm:text-2xl font-semibold text-gray-700">Advert Previews</h3>
      ...
      <div className="bg-white p-6 sm:p-10 rounded-xl shadow-lg">
        <h3 className="text-xl sm:text-2xl font-semibold text-gray-700 mb-6">Saved Campaigns</h3>
        {campaigns.length > 0 ? (
          <ul className="space-y-4">
            {campaigns.map((campaign) => (
              <li key={campaign._id} className="flex flex-col md:flex-row md:items-center justify-between p-4 sm:p-6 bg-slate-50 rounded-lg shadow-sm hover:shadow-md transition-shadow duration-300">
                <div className="flex-1">
                  <p className="text-base sm:text-lg font-medium text-gray-700">{campaign.campaignName}</p>
                  <p className="text-xs sm:text-sm text-gray-500 mt-1">
                    Created: {new Date(campaign.createdAt).toLocaleDateString()}
                    {campaign.status === 'sent' && campaign.sentAt && ` | Sent: ${new Date(campaign.sentAt).toLocaleDateString()}`}
                  </p>
                  <p className={`text-sm font-semibold mt-2 ${campaign.status === 'sent' ? 'text-teal-600' : 'text-orange-500'}`}>
                ...
                <div className="flex space-x-2 mt-4 md:mt-0">
                  ...
                  >
                    {isSending ? 'Sending...' : 'Send Ad to Clients'}
                  </button>
                </div>
              </li>
            ))}
          ...
          <p className="text-gray-500">No campaigns found. Create a new one above!</p>
        )}
      </div>
    </div>
  );
