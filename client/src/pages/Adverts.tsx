  ...
  const FilterButton: React.FC<{
    ...
    <button
      ...
      className={`px-4 py-2 text-sm font-medium rounded-full transition-colors ${
        currentFilter === status ? 'bg-primary text-white shadow' : 'bg-white text-slate-600 hover:bg-slate-100'
      }`}
    >
      {status}
    </button>
  ...
  return (
    <div className="space-y-6">
      <div className="flex flex-col md:flex-row justify-between md:items-center space-y-4 md:space-y-0">
        <h1 className="text-3xl font-bold text-slate-900">Adverts</h1>
      ...
      {filteredAdverts.length === 0 ? (
        <div className="text-center py-16">
          <p className="text-slate-500">No adverts found for the selected filters.</p>
        </div>
      ) : (
        <>
      ...
      <Modal isOpen={isFormModalOpen} onClose={handleCloseFormModal} title={editingAdvert ? 'Edit Advert' : 'Create New Advert'} size="xl">
      ...
      <Modal isOpen={!!selectedAdvert} onClose={handleCloseDetailModal} title={selectedAdvert?.title || 'Advert Details'} size="lg">
        <Suspense fallback={<div className="p-8"><LoadingSpinner /></div>}>
