  ...
  return (
    <div className="space-y-8">
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
        <Card className="p-4">
          <h2 className="text-lg font-semibold text-gray-800 mb-4">Advert Channels</h2>
          <ResponsiveContainer width="100%" height={300}>
            <PieChart>
              <Pie data={advertChannelData} dataKey="value" nameKey="name" cx="50%" cy="50%" outerRadius={100} label>
                {advertChannelData.map((_, index: number) => <Cell key={`cell-${index}`} fill={COLORS[index % COLORS.length]} />)}
              </Pie>
        ...
        <Card className="p-4">
          <h2 className="text-lg font-semibold text-gray-800 mb-4">Advert Status Distribution</h2>
          <ResponsiveContainer width="100%" height={300}>
            <PieChart>
              <Pie data={advertStatusData} dataKey="value" nameKey="name" cx="50%" cy="50%" innerRadius={60} outerRadius={100} paddingAngle={5} label>
                 {advertStatusData.map((_, index: number) => <Cell key={`cell-${index}`} fill={COLORS[index % COLORS.length]} />)}
              </Pie>
              <Tooltip content={<CustomTooltip />} />
               <Legend />
            </PieChart>
      ...
      <Card className="p-4">
        <h2 className="text-lg font-semibold text-gray-800 mb-4">Monthly Activity (New Clients vs Adverts)</h2>
        <ResponsiveContainer width="100%" height={400}>
            <BarChart data={activityByMonthData}>
                ...
                <Tooltip />
                <Legend />
                <Bar dataKey="clients" fill="#4F46E5" name="New Clients"/>
                <Bar dataKey="adverts" fill="#10B981" name="Adverts Created"/>
            </BarChart>
        </ResponsiveContainer>
      </Card>
      
       <Card className="p-4">
        <h2 className="text-lg font-semibold text-gray-800 mb-4">Advert Performance by Day</h2>
        <ResponsiveContainer width="100%" height={400}>
            <RadarChart cx="50%" cy="50%" outerRadius="80%" data={advertPerformanceData}>
                ...
                <Tooltip />
                <Legend />
                <Radar name="Sent" dataKey="sent" stroke="#4F46E5" fill="#4F46E5" fillOpacity={0.6} />
                <Radar name="Scheduled" dataKey="scheduled" stroke="#10B981" fill="#10B981" fillOpacity={0.6} />
            </RadarChart>
        </ResponsiveContainer>
      </Card>
    </div>
